...
try {
    ...
    dotnet publish src/Bingyu.App/Bingyu.App.csproj -c Release -r win-x64 --self-contained $taskSelfContained -p:SelfContained=$taskSelfContained -p:PublishSelfContained=$taskSelfContained -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:DebugType=None -p:DebugSymbols=false -o $taskOutput
